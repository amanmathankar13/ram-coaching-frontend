...
export function Classes() {
  ...
  // Map button class to full DB class key
  const clsMap = { '9':'9', '10':'10', '11':'11-PCM', '12':'12-PCM' }

  const loadClassData = async (c) => {
    setCls(c)
    setLoading(true)
    const fullCls = clsMap[c]
    try {
      ...
      {loading ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:'28px' }}>
          <div>
            ...
            {(subjects.length > 0 ? subjects : fallback.subjects).map(s => (
              <div key={s._id || s.name} className="card" style={{ marginBottom:'12px', borderColor:`${s.color}33` }}>
