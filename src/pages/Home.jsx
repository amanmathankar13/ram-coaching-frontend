...
export default function Home() {
  return (
    <div>
      {/* ── HERO ──────────────────────────────────────────────── */}
      <section style={{ padding: 'clamp(40px,6vw,80px) 0 clamp(32px,5vw,60px)' }}>
        <div className="container">
          <div style={{
            ...
            <div className="fade-up">
              ...
              <h1 style={{ fontSize:'clamp(32px,5vw,64px)', fontWeight:800, lineHeight:1.1, marginBottom:'18px' }}>
                Shape Your<br/>
                <span className="gradient-text">Future</span><br/>
                With Us
              </h1>

              <p style={{ fontSize:'clamp(14px,2vw,16px)', color:'var(--text2)', lineHeight:1.7, marginBottom:'28px', maxWidth:'480px' }}>
              ...
              <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
                ...
                <Link to="/jee"     className="btn btn-outline btn-lg">JEE / NEET →</Link>
              </div>
            ...
            {/* Right — class selector card */}
            <div className="card" style={{ position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', top:'-40px', right:'-40px', width:'140px', height:'140px', background:'radial-gradient(circle,rgba(108,99,255,.15),transparent)', borderRadius:'50%' }}/>
              <h3 style={{ fontFamily:'var(--font-head)', fontSize:'16px', fontWeight:700, marginBottom:'14px' }}>Select Your Class</h3>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', marginBottom:'12px' }}>
                {[['9th','Foundation'],['10th','Board Prep'],['11th','Science Stream'],['12th','Board + Entrance']].map(([num,sub]) => (
                  <Link key={num} to="/classes" style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'10px', padding:'12px', textAlign:'center', textDecoration:'none', display:'block', transition:'all .2s' }}
                  ...
                  >
                    <div style={{ fontFamily:'var(--font-head)', fontSize:'22px', fontWeight:700, color:'var(--accent)' }}>{num}</div>
                    <div style={{ fontSize:'11px', color:'var(--text3)', marginTop:'2px' }}>{sub}</div>
                  </Link>
                ))}
              </div>
              <div style={{ display:'flex', gap:'8px' }}>
                {[['⚡ JEE','Mains & Advanced','var(--gold)','/jee'],['🧬 NEET','UG Preparation','var(--green)','/neet']].map(([t,s,c,p]) => (
                  <Link key={t} to={p} style={{ flex:1, background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'8px', padding:'10px', textAlign:'center', textDecoration:'none', display:'block', transition:'border-color .2s' }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = c}
                    onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}
                  >
                    <div style={{ fontSize:'13px', fontWeight:700, color:c }}>{t}</div>
                    <div style={{ fontSize:'11px', color:'var(--text3)', marginTop:'2px' }}>{s}</div>
                  </Link>
                ))}
              </div>
            </div>
      ...
      <section style={{ padding:'40px 0 60px' }}>
        <div className="container">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:'18px' }}>
            {[
              { icon:'📖', title:'Class-wise Coaching', desc:'Structured curriculum for 9th–12th. Subject experts for each board subject.', link:'/classes', color:'rgba(108,99,255,.12)' },
              ...
              { icon:'🧬', title:'NEET Guidance',       desc:'Comprehensive NEET-UG prep with Biology focus, past papers & crash courses.', link:'/neet', color:'rgba(16,185,129,.12)' },
              { icon:'📝', title:'Daily DPP',           desc:'5 questions daily per subject — auto-graded with instant feedback.', link:'/dpp', color:'rgba(236,72,153,.12)' },
              { icon:'📹', title:'Video Lectures',      desc:'Recorded lectures and revision sessions inside the student dashboard.', link:'/login', color:'rgba(239,68,68,.12)' },
            ].map(item => (
              <div key={item.title} className="card card-hover">
                <div style={{ width:'44px', height:'44px', borderRadius:'10px', background:item.color, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', marginBottom:'14px' }}>{item.icon}</div>
                ...
                <Link to={item.link} style={{ display:'inline-flex', alignItems:'center', gap:'4px', marginTop:'12px', fontSize:'13px', color:'var(--accent)', fontWeight:500, textDecoration:'none' }}>Explore →</Link>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:'0 0 60px' }}>
        <div className="container">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:'18px', paddingTop:'20px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))', gap:'18px' }}>
            {[
              ...
              <div key={i} className="card">
                ...
                <div style={{ marginTop:'12px', fontWeight:600, fontSize:'14px' }}>— {t.name}, {t.role}</div>
                <div style={{ color:'var(--gold)', fontSize:'13px', marginTop:'4px' }}>★★★★★</div>
              </div>
            ))}
          </div>
          <div style={{ textAlign:'center', marginTop:'18px', color:'var(--text3)', fontSize:'13px' }}>⭐ 4.9 / 5 · 200+ Google Reviews</div>
        </div>
      </section>
    </div>
  )
