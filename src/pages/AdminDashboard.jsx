...
export default function AdminDashboard() {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      setStudents(all.data.filter(s => s.isApproved))
      setPending(all.data.filter(s => !s.isApproved))
    } catch {
      // demo data
      setStudents([
  ...
  const approve = async (id) => {
    ...
    const approved = pending.find(s => s._id === id)
    if (approved) setStudents(s => [...s, { ...approved, isApproved: true }])
  }
  ...
  const postNotice = async () => {
    if (!noticeForm.title || !noticeForm.body) return setFeedback('Please fill title and message.')
    try {
      const { data } = await api.post('/notices', noticeForm)
      setNotices(n => [data, ...n])
      setNoticeForm({ title:'', body:'', targetClass:'all' })
      setFeedback('✅ Notice posted!')
    } catch {
      setNotices(n => [{ _id: Date.now(), ...noticeForm, postedAt: new Date() }, ...n])
      setNoticeForm({ title:'', body:'', targetClass:'all' })
      setFeedback('✅ Notice posted!')
    }
  ...
  const uploadMaterial = async () => {
    if (!matForm.title || !matForm.driveLink) return setFeedback('Please fill all fields.')
    try {
      const { data } = await api.post('/materials', matForm)
      setMaterials(m => [...m, data])
    } catch {
      setMaterials(m => [...m, { _id: Date.now(), ...matForm }])
    }
    setMatForm({ title:'', subject:'Physics', class:'12-PCM', type:'pdf', driveLink:'' })
    setFeedback('✅ Material uploaded!')
    setTimeout(() => setFeedback(''), 3000)
  }
      ...
      <div className="dash-content">
        ...
        {section === 'overview' && (
          <div className="fade-in">
            ...
            <div className="stat-grid">
              <div className="stat-card"><div className="stat-val text-accent">{students.length}</div><div className="stat-lbl">Total Students</div></div>
              <div className="stat-card"><div className="stat-val text-gold">{pending.length}</div><div className="stat-lbl">Pending Approval</div></div>
              ...
              <div className="stat-card"><div className="stat-val text-cyan">{materials.length}</div><div className="stat-lbl">Materials Uploaded</div></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '8px' }}>
              ...
              <div className="notice" style={{ cursor:'pointer' }} onClick={() => setSection('inquiries')}>
                ...
                <div className="notice-body">Admission inquiries submitted via website. Review and follow up.</div>
              </div>
            </div>
          </div>
        )}
        ...
        {section === 'approvals' && (
          <div className="fade-in">
            ...
            {loading ? <Spinner /> : pending.length === 0 ? (
              <Alert type="success">✅ All registrations have been reviewed!</Alert>
            ) : pending.map(s => (
          ...
          <div className="fade-in">
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
              {['All','9','10','11-PCM','11-PCB','12-PCM','12-PCB'].map(c => (
                <button key={c} className="btn btn-sm btn-outline">{c === 'All' ? 'All Classes' : `Class ${c}`}</button>
              ))}
            </div>
            {students.map(s => (
              <div key={s._id} className="student-row">
        ...
        {section === 'materials' && (
          <div className="fade-in">
            ...
            <div style={{ background: 'var(--surface2)', borderRadius: 'var(--r)', border: '1px solid var(--border)', padding: '20px', marginBottom: '28px' }}>
              <div style={{ fontWeight: 600, marginBottom: '14px' }}>Upload New Material</div>
            ...
            {materials.map(m => (
              <div key={m._id} className="material-row">
                ...
                <div className="action-btns">
                  <button className="btn btn-sm btn-outline">Edit Access</button>
        ...
        {section === 'notices' && (
          <div className="fade-in">
            ...
            <div style={{ fontWeight: 600, marginBottom: '14px' }}>Recent Notices</div>
            {notices.map(n => (
