...
export default function StudentDashboard() {
  ...
  const fetchNotices = async () => {
    try {
      ...
      setNotices([
        { _id:'1', title:'📢 Mock Test — Saturday', body:'Full syllabus mock test on Saturday 22 March, 10 AM. Electrostatics, Magnetism, Current Electricity.', postedAt: new Date() },
        ...
        { _id:'3', title:'🏖 Holiday Notice',       body:'No classes on 25 March (Holi). Regular batches resume 26 March.', postedAt: new Date(Date.now()-86400000*4) },
      ])
    }
  }
        ...
        {section === 'overview' && (
          <div className="fade-in">
            ...
            <div className="stat-grid">
              ...
              <div className="stat-card"><div className="stat-val text-green">14</div><div className="stat-lbl">DPPs Done</div></div>
              ...
              <table>
                ...
                <tbody>
                  {[['18 Mar','Physics','4/5','done'],['17 Mar','Chemistry','3/5','done'],['16 Mar','Maths','5/5','done'],['15 Mar','Physics','2/5','done'],['19 Mar','Biology','—','pending']].map(([d,s,sc,st]) => (
                    <tr key={d+s}><td>{d}</td><td>{s}</td><td><strong>{sc}</strong></td>
                    <td><span className={`badge ${st==='done'?'badge-green':'badge-gold'}`}>{st==='done'?'Done':'Pending'}</span></td></tr>
                  ))}
                </tbody>
              </table>
            </div>
              ...
              <table>
                ...
                <tbody>
                  {SCHEDULE.map(([d,s,t,f]) => (
                    <tr key={d}><td><strong>{d}</strong></td><td>{s}</td>
                    <td style={{ color:'var(--text2)', fontSize:'13px' }}>{t}</td>
                    <td style={{ color:'var(--text3)', fontSize:'13px' }}>{f}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
              ...
              return (
                <>
                  ...
                  {CHAPTERS[selectedSubject].map(ch => {
                    const done = !!checkedChapters[`${selectedSubject}::${ch}`]
                    return (
                      <div key={ch} className="ch-row">
                        <div className={`ch-check ${done?'done':''}`} onClick={() => toggleChapter(selectedSubject, ch)}>{done?'✓':''}</div>
